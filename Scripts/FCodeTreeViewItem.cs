...
namespace CodeEditor
{
    ...
    public class FCodeTreeViewItem : CodeEditorTreeViewItem
    {
        ...
        public FCodeTreeViewItem(int depth = 0)
        {
            this.id = m_itemId ++;
            this.depth = depth;
            m_window = EditorWindow.GetWindow<FCodeWindow>();
        }
        ...
        public FCodeTreeViewItem(FCodeLuaFunction data, int depth) : this(depth)
        {
            ...
            m_window.icons.TryGetValue(m_fcodeNodeType.ToString(), out var itemIcon);
            icon = itemIcon;
        }
        ...
        ParsedTokens ParseTokens(List<Token> tokens)
        {
            ...
            {
                var prefix = (string) m_window.fcodeProperties["FCodeFunctionPrefix"];
            ...
            if (isFCodeFunction)
            {
                ...
                if (startIndex ++ >= 0 && m_window.fcodeLib.fcodeFunctions.TryGetValue(first, out var function))
                {
        ...
        public override void Draw()
        {
            ...
            switch (m_fcodeNodeType)
            {
                ...
                case FCodeNodeType.Vars:
                    ...
                    var index = m_window.fcodeTreeView.buildRoot.children.FindIndex(item =>
                    {
                ...
                case FCodeNodeType.Function:
                    ...
                    for(int parameterIndex = 0; parameterIndex < fcodeFunction.functionParameters.Count; ++parameterIndex)
                    {
                        ...
                        m_window.fcodeLib.parametersTypeRef.TryGetValue(paramName, out string parameterTypeRef);
                        ...
                        if(isConfig)
                        {
                            ...
                            {
                                i = m_window.fcodeConfig.FindIndex(p => p == parameterConfig.fields[0].Original());
                            }
                            var ni = EditorGUILayout.Popup(i, m_window.fcodeConfig.ToArray());
                            if (ni > -1)
                            {
                                if (ni != i)
                                {
                                    parameterConfig.fields = parameterConfig.fields ?? new List<Token>() { new NameToken("?") };
                                    parameterConfig.fields[0].SetOriginal(m_window.fcodeConfig[ni]);
                                }
                            }
                            else
                            {
            ...
            if (GUI.changed)
            {
                m_window.fcodeTreeView.changed = true;
                DisplayNameRefresh();
            }
        }
