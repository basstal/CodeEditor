...
namespace CodeEditor
{
    public class FCodeTreeView : CodeEditorTreeView<FCodeLogic>
    {
            ...
            if (buildRoot != null && buildRoot.children != null)
            {
                foreach (FCodeTreeViewItem item in buildRoot.children)
                {
                    if (item.fcodeNodeType == FCodeNodeType.Timing)
                    {
                        ...
                        sb.AppendLine("\t{");
                        sb.AppendLine($"\t\t-- {m_window.fcodeSignature["SingleLogicStart"]}");
                        ...
                        sb.AppendLine($"\t\t-- {m_window.fcodeSignature["SingleLogicEnd"]}");
                        sb.AppendLine("\t},");
                    }
                }

                sb.AppendLine("\tvars = ");
                sb.AppendLine($"\t-- {m_window.fcodeSignature["VarsStart"]}");
                sb.AppendLine("\t{");
                ...
                sb.AppendLine("\t},");
                sb.AppendLine($"\t-- {m_window.fcodeSignature["VarsEnd"]}");
            }
            sb.AppendLine("}");
            sb.AppendLine();
        ...
        protected override void ContextClickedItem(int id)
        {
            ...
            {
                var dict = m_window.fcodeLib.fcodeFunctions.OrderBy(o => o.Key);
